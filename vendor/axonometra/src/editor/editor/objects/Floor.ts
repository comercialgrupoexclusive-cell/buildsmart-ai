...
export class Floor extends Container {
  ...
  constructor(floorData?: FloorSerializable, previousFloor?: Floor) {
    ...
    if (floorData) {
      ...
      // BuildSmart P4.6 Bloco B — reaplica o status salvo por segmento
      // (chave `${minNodeId}-${maxNodeId}`, já canônica: addWall normaliza
      // leftNodeId < rightNodeId). Um plano v1 não tem esse campo (ou uma
      // parede pode faltar aqui se foi criada por outra via) — nesses casos
      // a parede fica no EXISTENTE padrão que a classe Wall já nasce com.
      if (floorData.wallSegmentStatus) {
        for (const [key, status] of Object.entries(
          floorData.wallSegmentStatus
        )) {
          const [left, right] = key.split('-').map(Number);
          this.wallNodeSequence.getWall(left, right)?.setStatus(status);
        }
      }
  ...
  public serialize(): FloorSerializable {
    ...
    // BuildSmart P4.6 Bloco B — status por segmento, mesma chave canônica
    // usada no load acima. Sempre grava mesmo quando EXISTENTE (padrão),
    // porque isso não é "ausência de dado" — é o estado real da parede.
    for (const wall of this.wallNodeSequence.getWalls()) {
      const key = `${wall.leftNode.getId()}-${wall.rightNode.getId()}`;
      plan.wallSegmentStatus[key] = wall.getStatus();
    }
    // furniture
