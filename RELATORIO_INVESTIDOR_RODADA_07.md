...
## Adendo — Correção de escopo do Web Search (commit `1e667ac`, main)

Revisão posterior identificou que a primeira implementação havia colocado
`web_search_preview` também em `lib/luizia-core.ts` (runtime geral da
Luiza), ampliando o escopo além do Investidor. Corrigido em `main` antes
deste merge:
