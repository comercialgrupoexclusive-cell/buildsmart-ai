...
export function FinancialDetailView({ context }: { context: PortalContextDTO }) {
  ...
  return <section className="space-y-6">
    ...
    {visibility.indicators && <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><MetricCard label="Valor orçado" value={money(financial.budget)} /><MetricCard label="Lançamentos confirmados" value={money(financial.realized)} tone="warning" /><MetricCard label="Pago" value={money(financial.paid)} detail="Materiais + mão de obra + gerenciamento" tone="success" /><MetricCard label="Saldo orçamentário" value={money(financial.balance)} /></div>}

    {visibility.indicators && <div className="card p-4 sm:p-5"><div className="mb-4"><h2 className="font-semibold">Detalhamento do pagamento</h2><p className="mt-1 text-xs" style={{ color: 'var(--text-secondary)' }}>Composição do valor &quot;Pago&quot; acima: materiais lançados + mão de obra e gerenciamento medidos.</p></div>
...
export function FinancingDetailView({ context }: { context: PortalContextDTO }) {
  ...
  // "Financiamento" (Caixa + FGTS) e Recursos Proprios sao conceitos
  // diferentes: recursos proprios sao contrapartida direta, nunca "recebida"
  // de uma fonte externa, entao nunca entram no percentual acima.
  const institutionalSources = financing.sources.filter(source => source.type !== 'recursos_proprios')
  return <section className="space-y-6"><div><p className="text-xs font-semibold uppercase" style={{ color: 'var(--text-secondary)' }}>Financiamento</p><h1 className="mt-1 text-3xl font-semibold">Fontes e reembolsos</h1><p className="mt-2 text-sm" style={{ color: 'var(--text-secondary)' }}>Financiamento Caixa e FGTS, acompanhados separadamente dos recursos proprios do cliente.</p></div><div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><MetricCard label="Base prevista (Caixa + FGTS)" value={money(financing.expected)} /><MetricCard label="Solicitado" value={money(financing.requested)} tone="warning" /><MetricCard label="Aprovado" value={money(financing.approved)} /><MetricCard label="Recebido" value={money(financing.received)} tone="success" /></div><div className="card p-4 sm:p-5"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase" style={{ color: 'var(--text-secondary)' }}>Progresso do financiamento</p><p className="mt-1 text-xl font-bold">{percent(receivedPercent)} recebido</p></div><p className="text-right text-sm font-semibold">Saldo<br />{money(financing.balance)}</p></div><Progress value={receivedPercent} /></div><div><div className="mb-3 flex items-center gap-2"><WalletCards size={18} style={{ color: 'var(--accent)' }} /><h2 className="text-lg font-semibold">Composicao do financiamento</h2></div><div className="grid gap-3 md:grid-cols-2">{institutionalSources.map(source => <article key={source.id} className="card p-4"><div className="flex items-center justify-between gap-3"><Landmark size={18} style={{ color: 'var(--accent)' }} /><span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{source.budgetName}</span></div><p className="mt-4 text-sm font-medium">{sourceLabel[source.type]}</p><p className="mt-1 text-xl font-bold">{money(source.value)}</p></article>)}</div></div>{financing.ownResources > 0 && <div className="card p-4 sm:p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase" style={{ color: 'var(--text-secondary)' }}>Recursos proprios</p><p className="mt-1 text-xs" style={{ color: 'var(--text-secondary)' }}>Contrapartida do cliente — nao entra no percentual de financiamento acima.</p></div><p className="shrink-0 text-xl font-bold">{money(financing.ownResources)}</p></div></div>}<div><div className="mb-3 flex items-center gap-2"><CheckCircle2 size={18} style={{ color: 'var(--accent)' }} /><h2 className="text-lg font-semibold">Historico de reembolsos</h2></div>{financing.reimbursements.length ? <div className="space-y-3">{financing.reimbursements.map(item => <StatusItemCard key={item.id} title={item.title} eyebrow={item.status} value={money(item.received || item.approved || item.requested)} meta={<span>{date(item.date)}</span>} />)}</div> : <EmptyState title="Nenhum reembolso registrado" description="Solicitacoes, aprovacoes e recebimentos aparecerao aqui." />}</div></section>
}
