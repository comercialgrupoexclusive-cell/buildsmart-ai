...
export function EvolutionView({ context }: { context: PortalContextDTO }) {
  ...
  return <section className="space-y-6">
    ...
    {visibility.indicators && <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><MetricCard label="Avanço físico" value={percent(axes.physical)} detail={`Medição · ${completed} de ${stages.length} etapas concluídas`} tone="success" /><MetricCard label="Mão de obra" value={percent(axes.labor)} detail="Medição de mão de obra" /><MetricCard label="Realização financeira" value={percent(axes.financial)} detail="Lançamentos sobre o orçamento" tone="warning" /><MetricCard label="Financiamento recebido" value={percent(axes.financing)} detail="Sobre os recursos previstos" /></div>}
...
export function OverviewCharts({ context }: { context: PortalContextDTO }) {
  ...
  return <div className="card min-w-0 overflow-hidden"><div className="p-4 pb-0 sm:p-5 sm:pb-0"><h2 className="font-semibold">Evolução físico-financeira</h2><p className="mt-1 text-xs" style={{ color: 'var(--text-secondary)' }}>Medições realizadas e a próxima previsão do cronograma Caixa</p></div><div className="h-72 p-2 sm:p-4"><ComposedChart responsive data={data} style={{ width: '100%', height: '100%' }} margin={{ top: 12, right: 2, left: -20, bottom: 0 }}><CartesianGrid stroke="var(--border)" vertical={false} /><XAxis dataKey="label" tick={{ fill: 'var(--text-secondary)', fontSize: 10 }} axisLine={false} tickLine={false} /><YAxis yAxisId="value" tickFormatter={compactMoney} tick={{ fill: 'var(--text-secondary)', fontSize: 10 }} axisLine={false} tickLine={false} width={66} /><YAxis yAxisId="physical" orientation="right" domain={[0, 100]} tickFormatter={value => `${value}%`} tick={{ fill: 'var(--text-secondary)', fontSize: 10 }} axisLine={false} tickLine={false} width={38} /><Tooltip formatter={(value, name) => name === 'Avanço físico' ? `${Number(value).toLocaleString('pt-BR', { maximumFractionDigits: 3 })}%` : money(Number(value))} contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8 }} /><Legend wrapperStyle={{ fontSize: 11 }} /><Bar yAxisId="value" dataKey="value" name="Valor acumulado" fill="var(--accent)" fillOpacity={0.18} stroke="var(--accent)" radius={[4, 4, 0, 0]} /><Line yAxisId="physical" type="monotone" dataKey="physical" name="Avanço físico" stroke="var(--success)" strokeWidth={2.5} dot={{ r: 3 }} /></ComposedChart></div><div className="divide-y border-t" style={{ borderColor: 'var(--border)' }}>{data.map(item => <div key={item.month} className="grid grid-cols-[1fr_auto] items-center gap-3 px-4 py-3 text-sm sm:grid-cols-[110px_1fr_auto] sm:px-5"><div className="flex items-center gap-2"><span className="font-semibold capitalize">{month(item.month)}/{item.month.slice(2, 4)}</span>{item.kind === 'forecast' && <Badge variant="info">Previsão</Badge>}</div><span className="hidden text-xs sm:block" style={{ color: 'var(--text-secondary)' }}>{item.kind === 'forecast' ? 'Próxima medição' : 'Realizado'}</span><div className="text-right"><p className="font-bold tabular-nums">{Number(item.physical).toLocaleString('pt-BR', { maximumFractionDigits: 3 })}%</p><p className="mt-0.5 text-xs tabular-nums" style={{ color: 'var(--text-secondary)' }}>{money(item.value)}</p></div></div>)}</div></div>
}
  ...
  return <section className="space-y-6">
    ...
    {visibility.indicators && <div className="card p-4 sm:p-5"><div className="mb-4"><h2 className="font-semibold">Pagamento operacional</h2><p className="mt-1 text-xs" style={{ color: 'var(--text-secondary)' }}>Materiais lançados + mão de obra e gerenciamento medidos — mesmo total da Visão Geral, nunca somado ao &quot;Pago (lançamentos)&quot; acima.</p></div>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <MetricCard label="Materiais pagos" value={money(operational.materials.paid)} detail={`de ${money(operational.materials.planned)} previstos`} />
        <MetricCard label="Mão de obra paga" value={money(operational.labor.paid)} detail="Medição de mão de obra" />
        <MetricCard label="Gerenciamento pago" value={money(operational.management.paid)} detail="Medição de gerenciamento" />
        <MetricCard label="Equipamentos" value={money(operational.otherEquipmentPaid)} detail="Fora do total operacional" />
      </div>
      <p className="mt-4 text-sm"><strong>Total operacional pago: {money(operational.total.paid)}</strong> de {money(operational.total.planned)} previstos</p>
    </div>}

    {visibility.chart && chartData.length > 0 && <div className="card min-w-0 p-4 sm:p-5"><div className="mb-5"><h2 className="font-semibold">Evolução mensal</h2><p className="mt-1 text-xs" style={{ color: 'var(--text-secondary)' }}>Lançamentos confirmados e pagos (compra_itens) por mês</p></div><div className="h-64"><BarChart responsive data={chartData} style={{ width: '100%', height: '100%' }} margin={{ top: 4, right: 4, left: -16 }}><CartesianGrid stroke="var(--border)" vertical={false} /><XAxis dataKey="label" tick={{ fill: 'var(--text-secondary)', fontSize: 11 }} axisLine={false} tickLine={false} /><YAxis tickFormatter={compactMoney} tick={{ fill: 'var(--text-secondary)', fontSize: 10 }} axisLine={false} tickLine={false} width={62} /><Tooltip formatter={value => money(Number(value))} contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8 }} /><Bar dataKey="realized" name="Confirmado" fill="var(--accent)" radius={[4, 4, 0, 0]} /><Bar dataKey="paid" name="Pago" fill="var(--success)" radius={[4, 4, 0, 0]} /></BarChart></div></div>}
