    ...
    <div className="relative">
      ...
      {rows.map(row => {
        ...
        return <div key={`${row.level}-${row.id}`} className="border-b px-4 py-3 last:border-b-0" style={{ borderColor: 'var(--border)', background: row.level ? 'var(--bg-secondary)' : undefined }}>
          <div className="flex min-w-0 items-start gap-2">{canExpand ? <button type="button" onClick={() => setExpanded(current => { const next = new Set(current); if (next.has(row.id)) next.delete(row.id); else next.add(row.id); return next })} className="mt-0.5 grid size-7 shrink-0 place-items-center rounded-md" style={{ color: 'var(--text-secondary)' }} aria-label={open ? 'Recolher' : 'Expandir'}>{open ? <ChevronDown size={16} /> : <ChevronRight size={16} />}</button> : <span className="w-7 shrink-0" />}<div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><p className={`truncate text-sm ${row.level ? 'font-medium' : 'font-semibold'}`}>{row.nome}</p>{row.percentual != null ? <span className="shrink-0 whitespace-nowrap text-xs font-semibold tabular-nums" style={{ color: row.percentual >= 100 ? 'var(--success)' : 'var(--accent)' }}>{formatPercent(row.percentual)}</span> : <span className="shrink-0 text-xs" style={{ color: 'var(--text-secondary)' }}>Planejado</span>}</div><p className="mt-0.5 text-xs" style={{ color: 'var(--text-secondary)' }}>{shortDate(row.inicio)} → {shortDate(row.fim)}</p></div></div>
          <div className="relative mt-3 h-7 overflow-hidden rounded-md" style={{ background: 'color-mix(in srgb, var(--border) 55%, transparent)' }}>{start !== null && end !== null ? <div className="absolute inset-y-1 rounded-sm" style={{ left: `${left}%`, width: `${width}%`, background: row.level ? 'color-mix(in srgb, var(--accent) 68%, white)' : 'var(--accent)' }}>{row.percentual != null && <div className="h-full rounded-sm" style={{ width: `${Math.min(100, row.percentual)}%`, background: 'color-mix(in srgb, var(--success) 72%, transparent)' }} />}</div> : <span className="absolute inset-0 grid place-items-center text-[11px]" style={{ color: 'var(--text-secondary)' }}>Datas ainda não definidas</span>}</div>
        </div>
      })}
    </div>
  </div>
