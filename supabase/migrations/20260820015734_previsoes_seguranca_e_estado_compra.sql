-- Fecha duas lacunas encontradas na validacao final de Previsoes/Alertas:
--
-- 1) obra_previsao_save e obra_previsao_undo_last sao SECURITY DEFINER mas
--    nao validavam p_profile_id -- qualquer chamada com um profile_id
--    (mesmo invalido/de outro usuario) conseguia gravar/desfazer previsoes.
--    Adiciona a mesma checagem de autorizacao ja usada em outras RPCs
--    privilegiadas do sistema (perfil precisa existir e ser admin/usuario).
--
-- 2) obra_previsoes_list ganha 'vinculoEstruturalId', 'compraVinculada' e
--    'compraRecebida', calculados sempre por vinculo estrutural contra
--    compra_itens (orcamento_item_id / subetapa_orcamento_item_id gravados
--    em metadados.origemCronograma no momento em que a previsao foi criada
--    a partir de uma sugestao) -- nunca por comparacao de nome. Quando a
--    previsao nao tem nenhum vinculo estrutural gravado (baseline
--    importado, previsao manual, ou previsao anterior a esta
--    funcionalidade), compraVinculada retorna null ("vinculo nao
--    identificado"), nunca false ("nao comprado"): sao estados diferentes.
--
-- obra_previsao_sugestoes ganha 'orcamentoItemId' dentro de
-- origemCronograma (alem do 'subetapaOrcamentoItemId' ja existente), para
-- que o vinculo estrutural completo fique disponivel para gravacao em
-- obra_previsao_save quando o usuario aceitar a sugestao.
--
-- Por fim, tranca as 4 RPCs de previsao para service_role, seguindo o
-- mesmo padrao ja aplicado as RPCs administrativas do Portal/Feed
-- (lock_down_portal_admin_rpcs): o unico chamador legitimo e o proxy
-- app/api/portal-admin/route.ts, que injeta o profile_id da sessao
-- assinada (nunca o que o cliente manda no corpo).
...
create or replace function public.obra_previsao_undo_last(p_obra_id uuid, p_profile_id uuid default null::uuid)
 returns jsonb
 language plpgsql
 security definer
 set search_path to ''
...
select coalesce(jsonb_agg(jsonb_build_object(
  ...
  'vinculoEstruturalId', nullif(coalesce(p.metadados->'origemCronograma'->>'orcamentoItemId', p.metadados->'origemCronograma'->>'subetapaOrcamentoItemId'), ''),
  'compraVinculada', case
    when (p.metadados->'origemCronograma'->>'orcamentoItemId') is null and (p.metadados->'origemCronograma'->>'subetapaOrcamentoItemId') is null then null
    else exists (
      select 1 from public.compra_itens ci
      where ci.obra_id = p.obra_id
        and (
          (nullif(p.metadados->'origemCronograma'->>'orcamentoItemId','') is not null and ci.orcamento_item_id = nullif(p.metadados->'origemCronograma'->>'orcamentoItemId','')::uuid)
          or (nullif(p.metadados->'origemCronograma'->>'subetapaOrcamentoItemId','') is not null and ci.subetapa_orcamento_item_id = nullif(p.metadados->'origemCronograma'->>'subetapaOrcamentoItemId','')::uuid)
        )
    )
  end,
  'compraRecebida', exists (
    select 1 from public.compra_itens ci
    where ci.obra_id = p.obra_id
      and ci.status_recebimento = 'recebido'
      and (
        (nullif(p.metadados->'origemCronograma'->>'orcamentoItemId','') is not null and ci.orcamento_item_id = nullif(p.metadados->'origemCronograma'->>'orcamentoItemId','')::uuid)
        or (nullif(p.metadados->'origemCronograma'->>'subetapaOrcamentoItemId','') is not null and ci.subetapa_orcamento_item_id = nullif(p.metadados->'origemCronograma'->>'subetapaOrcamentoItemId','')::uuid)
      )
...
revoke execute on function public.obra_previsao_sugestoes(uuid, text, integer) from anon, authenticated, public;
revoke execute on function public.obra_previsao_save(uuid, uuid, jsonb, uuid) from anon, authenticated, public;
revoke execute on function public.obra_previsao_undo_last(uuid, uuid) from anon, authenticated, public;
revoke execute on function public.obra_previsoes_list(uuid, text) from anon, authenticated, public;

grant execute on function public.obra_previsao_sugestoes(uuid, text, integer) to service_role;
grant execute on function public.obra_previsao_save(uuid, uuid, jsonb, uuid) to service_role;
grant execute on function public.obra_previsao_undo_last(uuid, uuid) to service_role;
grant execute on function public.obra_previsoes_list(uuid, text) to service_role;
