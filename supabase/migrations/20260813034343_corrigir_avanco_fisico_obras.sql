-- O card da obra e a visao geral devem usar o mesmo avanco fisico da medicao:
-- primeiro o peso fisico configurado; na ausencia dele, o peso financeiro.
...
as $$
  with orcamentos_ativos as (
    ...
    select
      oa.obra_id,
      i.etapa_id,
      sum(coalesce(i.valor_total_informado_snapshot, i.quantidade * i.preco_unitario_snapshot, 0)) as valor
  ...
  select
    c.obra_id,
    round(coalesce(p.avanco_fisico, 0), 2) as avanco_fisico,
    c.orcamentos_ativos
  from contagem c
