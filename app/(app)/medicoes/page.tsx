...
export default function MedicoesPage() {
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Seletor de obra — esta página espelha a aba "Diário / Medições" da obra, permitindo trocar de obra aqui */}
      <div className="flex flex-col lg:flex-row gap-3 lg:items-end justify-between">
        ...
        {obraAtual && (
          <Link href={`/obras/${obraAtual.id}?tab=medicoes`} className="inline-flex items-center gap-2 text-sm font-medium hover:opacity-80" style={{ color: 'var(--accent)' }}>
      ...
      {loadingObras ? (
        ...
        <EmptyState icon={ClipboardList} title="Selecione uma obra" description="Escolha uma obra acima para acompanhar o avanço físico por etapa e subetapa." />
      ) : (
        <ObraMedicoes key={obraId} obraId={obraId} />
      )}
    </div>
  )
}
