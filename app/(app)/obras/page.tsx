      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        <div className="flex gap-1 p-1 rounded-lg flex-wrap" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          {STATUS_FILTROS.map(s => {
            const CORES: Record<string, string> = { ativa: '#10B981', orcamento: '#F59E0B', concluida: '#3B7BF8', paralisada: '#EF4444' }
            const cor = CORES[s] || 'var(--accent)'
            return (
              <button
                key={s}
                onClick={() => setFiltro(s)}
                ...
                style={filtro === s
                  ? { background: s === 'todas' ? 'var(--accent)' : cor, color: 'white' }
